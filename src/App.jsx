...
async function findOrCreateDatabase() {
  ...
  if (search.results && search.results.length > 0) return search.results[0].id;
  const db = await notionRequest("/databases", "POST", {
    ...
    properties: {
      Название: { title: {} },
      Тип: { select: { options: [
        { name: "Статья", color: "blue" }, { name: "PDF", color: "red" },
        { name: "Заметка", color: "yellow" }, { name: "Видео", color: "purple" },
        { name: "НПА", color: "green" }, { name: "Письмо", color: "orange" },
      ]}},
      Источник: { select: { options: [
        { name: "Telegram", color: "blue" }, { name: "Email", color: "green" },
        { name: "Интернет", color: "gray" }, { name: "Свой файл", color: "yellow" },
      ]}},
      Статус: { select: { options: [
        { name: "Новое", color: "gray" }, { name: "Изучаю", color: "yellow" }, { name: "Изучено", color: "green" },
      ]}},
...
async function fetchItems(dbId, filters = {}) {
  const and = [];
  if (filters.status && filters.status !== "Все") and.push({ property: "Статус", select: { equals: filters.status } });
  if (filters.type && filters.type !== "Все") and.push({ property: "Тип", select: { equals: filters.type } });
  if (filters.source && filters.source !== "Все") and.push({ property: "Источник", select: { equals: filters.source } });
  const body = { sorts: [{ timestamp: "created_time", direction: "descending" }], ...(and.length ? { filter: { and } } : {}) };
...
async function createItem(dbId, item) {
  ...
  if (item.tags && item.tags.length) props["Теги"] = { multi_select: item.tags.map(t => ({ name: t })) };
  return notionRequest("/pages", "POST", { parent: { database_id: dbId }, properties: props });
}

async function updateStatus(pageId, status) {
  return notionRequest(`/pages/${pageId}`, "PATCH", { properties: { Статус: { select: { name: status } } } });
}
...
const TYPE_ICONS = { Статья: "📄", PDF: "📕", Заметка: "📝", Видео: "🎬", НПА: "⚖️", Письмо: "✉️" };
const SOURCE_ICONS = { Telegram: "✈️", Email: "📧", Интернет: "🌐", "Свой файл": "💾" };
const STATUS_COLORS = { Новое: "#6b7280", Изучаю: "#d97706", Изучено: "#16a34a" };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (sharedTitle || sharedUrl) {
      setFormData(f => ({ ...f, title: sharedTitle, url: sharedUrl, source: "Интернет" }));
      setShowForm(true);
  ...
  const handleStatusCycle = async (item) => {
    ...
    try { await updateStatus(item.id, next); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f6fb", color: "#1a1d2e", fontFamily: "'Golos Text', 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        .card { background: #ffffff; border: 1px solid #e2e5f0; border-radius: 12px; transition: all .2s; cursor: pointer; }
        .card:hover { border-color: #3b4fd8; transform: translateY(-2px); box-shadow: 0 8px 32px rgba(59,79,216,.1); }
        ...
        .input { background: #f0f2f8; border: 1px solid #e2e5f0; border-radius: 8px; color: #1a1d2e; font-family: inherit; font-size: 14px; padding: 10px 14px; width: 100%; outline: none; transition: border .15s; }
        .input:focus { border-color: #3b4fd8; }
        .select { appearance: none; cursor: pointer; }
        .overlay { position: fixed; inset: 0; background: rgba(0,0,0,.4); backdrop-filter: blur(4px); z-index: 100; display: flex; align-items: center; justify-content: center; padding: 16px; }
        .modal { background: #ffffff; border: 1px solid #e2e5f0; border-radius: 16px; padding: 28px; width: 100%; max-width: 520px; max-height: 90vh; overflow-y: auto; }
        .flt-btn { background: #f0f2f8; border: 1px solid #e2e5f0; color: #6b7280; font-size: 13px; font-weight: 500; padding: 6px 14px; border-radius: 20px; cursor: pointer; transition: all .15s; font-family: inherit; }
      ...
      {/* Header */}
      <div style={{ background: "#ffffff", borderBottom: "1px solid #e8eaf2", padding: "0 20px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 15, fontWeight: 700 }}>МОПБ Хаб</div>
              <div style={{ fontSize: 11, color: "#9ca3af" }}>База знаний · Notion</div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button className="btn" onClick={() => setView(v => v === "grid" ? "list" : "grid")}
              style={{ background: "#f0f2f8", color: "#6b7280", padding: "8px 14px", fontSize: 13 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px" }}>
        ...
        {/* Filters */}
        <div style={{ background: "#ffffff", border: "1px solid #e2e5f0", borderRadius: 12, padding: "14px 18px", marginBottom: 20 }}>
          <input className="input" placeholder="🔍  Поиск по названию или заметке..."
            value={filters.search} onChange={e => setFilters(f => ({ ...f, search: e.target.value }))}
            style={{ marginBottom: 14 }} />
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 12, color: "#9ca3af", alignSelf: "center", minWidth: 52 }}>Статус:</span>
            {STATUSES.map(s => (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 12, color: "#9ca3af", alignSelf: "center", minWidth: 52 }}>Тип:</span>
            {TYPES.map(s => (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            <span style={{ fontSize: 12, color: "#9ca3af", alignSelf: "center", minWidth: 52 }}>Источник:</span>
            {SOURCES.map(s => (
        ...
        {/* Error */}
        {error && (
          <div style={{ background: "#fff0f0", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#dc2626", display: "flex", justifyContent: "space-between" }}>
            <span>⚠️ {error}</span>
            ...
            {filtered.map(item => (
              <div key={item.id} className="card fadein" onClick={() => setActiveItem(item)} style={{ padding: "18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  ...
                  <span className="pill" style={{ background: STATUS_COLORS[item.status] + "22", color: STATUS_COLORS[item.status] }}>{item.status}</span>
                </div>
                <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6, lineHeight: 1.4 }}>{item.title}</div>
                {item.notes && <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 10, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{item.notes}</div>}
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {item.source && <span className="pill" style={{ background: "#f0f2f8", color: "#6b7280" }}>{SOURCE_ICONS[item.source]} {item.source}</span>}
                  {item.tags.slice(0, 2).map(t => <span key={t} className="pill" style={{ background: "#eef0fb", color: "#3b4fd8" }}>#{t}</span>)}
                </div>
              </div>
            ))}
          </div>
            ...
            {filtered.map(item => (
              <div key={item.id} className="card fadein" onClick={() => setActiveItem(item)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {item.notes && <div style={{ fontSize: 12, color: "#9ca3af", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.notes}</div>}
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  {item.source && <span className="pill" style={{ background: "#f0f2f8", color: "#6b7280", fontSize: 11 }}>{item.source}</span>}
        ...
        {!loading && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginTop: 24 }}>
            {[
              { label: "Всего материалов", val: stats.total, color: "#3b4fd8" },
              { label: "Новые", val: stats.new, color: "#6b7280" },
              { label: "Изучаю", val: stats.learning, color: "#d97706" },
              { label: "Изучено", val: stats.done, color: "#16a34a" },
            ].map(s => (
              <div key={s.label} style={{ background: "#ffffff", border: "1px solid #e2e5f0", borderRadius: 12, padding: "16px 18px" }}>
        ...
        {/* Progress */}
        {stats.total > 0 && (
          <div style={{ background: "#ffffff", border: "1px solid #e2e5f0", borderRadius: 12, padding: "14px 18px", marginTop: 12 }}>
            ...
            <div style={{ background: "#f0f2f8", borderRadius: 8, height: 8, overflow: "hidden" }}>
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal fadein">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Название *</label>
                <input className="input" placeholder="Название материала" value={formData.title}
                  onChange={e => setFormData(f => ({ ...f, title: e.target.value }))} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Тип</label>
                ...
                <div>
                  <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Источник</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Ссылка (URL)</label>
                <input className="input" placeholder="https://..." value={formData.url}
                  onChange={e => setFormData(f => ({ ...f, url: e.target.value }))} />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Теги (через запятую)</label>
                <input className="input" placeholder="пожарная безопасность, ГОСТ, охрана труда" value={formData.tags}
                  onChange={e => setFormData(f => ({ ...f, tags: e.target.value }))} />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Заметки</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: "#9ca3af", marginBottom: 6, display: "block" }}>Статус</label>
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && setActiveItem(null)}>
          <div className="modal fadein">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
              ...
              <button onClick={() => setActiveItem(null)} style={{ background: "none", border: "none", color: "#9ca3af", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              ...
              {activeItem.type && <span className="pill" style={{ background: "#f0f2f8", color: "#6b7280" }}>{TYPE_ICONS[activeItem.type]} {activeItem.type}</span>}
              {activeItem.source && <span className="pill" style={{ background: "#f0f2f8", color: "#6b7280" }}>{SOURCE_ICONS[activeItem.source]} {activeItem.source}</span>}
            </div>
            {activeItem.notes && (
              <div style={{ background: "#f4f6fb", borderRadius: 10, padding: "14px", marginBottom: 16, fontSize: 14, lineHeight: 1.6, color: "#3a3d52" }}>
                {activeItem.notes}
              </div>
            )}
            {activeItem.tags.length > 0 && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
                {activeItem.tags.map(t => <span key={t} className="pill" style={{ background: "#eef0fb", color: "#3b4fd8" }}>#{t}</span>)}
              </div>
            )}
            {activeItem.url && (
              <a href={activeItem.url} target="_blank" rel="noreferrer"
                style={{ display: "block", color: "#3b4fd8", fontSize: 13, marginBottom: 16, wordBreak: "break-all" }}>
                🔗 {activeItem.url}
              </a>
            )}
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              <button className="btn" onClick={() => { handleStatusCycle(activeItem); setActiveItem(a => ({ ...a, status: STATUS_NEXT[a.status] })); }}
                style={{ flex: 1, background: "#f0f2f8", color: "#1a1d2e", padding: "10px", fontSize: 13 }}>
                ↻ Статус: {STATUS_NEXT[activeItem.status]}
              </button>
              <button className="btn" onClick={() => handleDelete(activeItem.id)}
                style={{ background: "#fff0f0", color: "#dc2626", padding: "10px 16px", fontSize: 13 }}>
                🗑
              </button>
            </div>
          </div>
