    ...
    <div style={{ minHeight: "100vh", background: "#f4f6fb", color: "#1a1d2e", fontFamily: "'Golos Text', 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: #e8eaf2; } ::-webkit-scrollbar-thumb { background: #c8ccd8; border-radius: 3px; }
      ...
      {/* Header */}
      <div style={{ background: "#ffffff", borderBottom: "1px solid #1e2336", padding: "0 20px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button className="btn" onClick={() => setView(v => v === "grid" ? "list" : "grid")}
              style={{ background: "#f0f2f8", color: "#9ca3af", padding: "8px 14px", fontSize: 13 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Новые", val: stats.new, icon: "🔵", color: "#9ca3af" },
        ...
        {/* Progress bar */}
        {stats.total > 0 && (
          <div style={{ background: "#ffffff", border: "1px solid #e2e5f0", borderRadius: 12, padding: "14px 18px", marginBottom: 24 }}>
            ...
            {filtered.map(item => (
              <div key={item.id} className="card fadein" onClick={() => setActiveItem(item)} style={{ padding: "18px" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: "auto" }}>
                  {item.source && <span className="pill" style={{ background: "#f0f2f8", color: "#9ca3af" }}>{SOURCE_ICONS[item.source]} {item.source}</span>}
                  {item.tags.slice(0, 2).map(t => <span key={t} className="pill" style={{ background: "#eef0fb", color: "#818cf8" }}>#{t}</span>)}
                </div>
              </div>
            ))}
          </div>
            ...
            {filtered.map(item => (
              <div key={item.id} className="card fadein" onClick={() => setActiveItem(item)}
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  {item.source && <span className="pill" style={{ background: "#f0f2f8", color: "#9ca3af", fontSize: 11 }}>{item.source}</span>}
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && setActiveItem(null)}>
          <div className="modal fadein">
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              ...
              {activeItem.type && <span className="pill" style={{ background: "#f0f2f8", color: "#9ca3af" }}>{TYPE_ICONS[activeItem.type]} {activeItem.type}</span>}
              {activeItem.source && <span className="pill" style={{ background: "#f0f2f8", color: "#9ca3af" }}>{SOURCE_ICONS[activeItem.source]} {activeItem.source}</span>}
            </div>
            {activeItem.notes && (
              <div style={{ background: "#f0f2f8", borderRadius: 10, padding: "14px", marginBottom: 16, fontSize: 14, lineHeight: 1.6, color: "#3a3d52" }}>
                {activeItem.notes}
              </div>
            )}
            {activeItem.tags.length > 0 && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
                {activeItem.tags.map(t => <span key={t} className="pill" style={{ background: "#eef0fb", color: "#818cf8" }}>#{t}</span>)}
              </div>
            )}
            {activeItem.url && (
